...
func permitted(building: Building, type: UnitConfigs.UnitType) -> bool:
	...
	if building.faction=="goblin":
		...
		return building.building_type in [BuildingConfigs.BuildingType.BARRACKS,BuildingConfigs.BuildingType.WORKSHOP] and type in [UnitConfigs.UnitType.GOBLIN_WARRIOR,UnitConfigs.UnitType.GOBLIN_ARCHER,UnitConfigs.UnitType.GOBLIN_SPEARMAN,UnitConfigs.UnitType.SPIDER_RIDER,UnitConfigs.UnitType.TROLL]
	...
	return building.building_type in [BuildingConfigs.BuildingType.BARRACKS,BuildingConfigs.BuildingType.WORKSHOP] and type in [UnitConfigs.UnitType.WARRIOR,UnitConfigs.UnitType.ARCHER]
...
func enqueue(building: Building, type: UnitConfigs.UnitType) -> String:
	...
	var cost: Dictionary = UnitConfigs.get_config(type).get("cost",{})
	...
	for b in game.all_buildings.duplicate():
		...
		if b.rally_point!=Vector3.INF:
			if UnitConfigs.is_vessel(unit.unit_type):game.transport_system.order_ship(unit,b.rally_point)
				...
				unit.set_path(game.grid_manager.find_path(unit.position,b.rally_point,b.faction))
...
func set_rally(building: Building, destination: Vector3) -> bool:
	if not is_instance_valid(building) or building.faction!="player" or not building.is_constructed:return false
	if building.building_type==BuildingConfigs.BuildingType.PORT:
		...
		var path: Array[Vector3] = game.grid_manager.find_path(building.position,destination)
	...
	if not is_instance_valid(building.rally_marker):
		...
		var pole: MeshInstance3D = VoxelMeshFactory.create_box_mesh(Vector3(0.06,1.3,0.06),Color("e9c478"))
		pole.position.y=0.65;marker.add_child(pole)
		var flag: MeshInstance3D = VoxelMeshFactory.create_box_mesh(Vector3(0.65,0.38,0.06),Color("5ee0ac"))
		flag.position=Vector3(0.3,1.1,0);marker.add_child(flag)
